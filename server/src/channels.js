module.exports = function (app) {
  ...
  app.service('todolists').publish(list => {
    return app.channel(`lists/${list._id}`);
  });
  ...
  app.service('todolists').on('created', list => {
    ...
    const { connections } = app.channel(app.channels).filter(connection =>
      list.access.includes(connection.user.email)
    );
  ...
  function onUpdateList(list) {
    console.log('list updated, publishing');
  ...
  app.service('todos').publish(todo => {
    return app.channel(`lists/${todo.listId}`);
  });
};
